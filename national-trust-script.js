...
console.log(responseData.list[0].weather)
console.log(responseData.list[1].weather)
const cleanDescriptionToday = responseData.list[0].weather[0].description.replace(/['"]+/g, '')
console.log(cleanDescriptionToday)
...
console.log(cleanDescriptionTomorrow)
...
let imgTag1 = $('#today-weather-icon');
        if (imgTag1.length === 0) {
            // If img tag does not exist, create it
...
console.log("tomorrowIconUrl", tomorrowIconUrl)
let imgTag2 = $('#tomorrow-weather-icon');
        ...
        // Set the src attribute of the img tag to the icon URL
        imgTag1.attr('src', tomorrowIconUrl);
});
